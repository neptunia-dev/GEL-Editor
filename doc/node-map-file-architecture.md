...
> 状态：新模型设计阶段。
>
> 旧版 Node Map 领域代码已经删除，当前尚未实现新的 Node Map 模型。本文是下一版实现的
> 规范，不描述兼容层，也不要求保留旧文件格式。
> `workspace/node_map/` 已提供独立的 [显示占位](node-map-preview.md)，它只使用静态
> GraphEdit/GraphNode 演示场景，不实现本文的模型、序列化和编译协议。
>
> 目标：建立一个类似 ComfyUI 的节点图系统。所有可放置在画布中、可以参与连接或执行
> 编译的对象都继承同一个模型父类；`Scene` 是普通节点，同时是一个可以展开编辑的复合节点。
