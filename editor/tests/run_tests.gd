...
const NodeMapSerializer = preload("res://node_map/persistence/node_map_serializer.gd")
...
func _test_node_map_serializer() -> void:
    var source := SceneNode.new(
        "serializer-source", "serializer-source", "Serializer", "scenes/serializer-source/main.lua",
        [], [ExitPort.new("next-port", "next")], Vector2(32, 64),
    )
    var target := SceneNode.new("serializer-target", "serializer-target")
    var map := SceneMap.new()
    _check(map.add_node(source) and map.add_node(target), "serializer map accepts nodes")
    _check(map.set_entry_node("serializer-source"), "serializer entry set")
    _check(map.add_route(RouteEdge.new("serializer-edge", "serializer-source", "next-port", "serializer-target")), "serializer route added")
    var decoded := NodeMapSerializer.deserialize(NodeMapSerializer.serialize(map))
    _check(decoded["ok"], "serializer round trip succeeds")
    var restored: SceneMap = decoded["scene_map"]
    _check(restored.get_node("serializer-source").position == Vector2(32, 64), "serializer restores position")
    _check(restored.get_node("serializer-source").get_exit("next-port") != null, "serializer restores exits")
    _check(restored.get_route("serializer-edge") != null, "serializer restores route")
